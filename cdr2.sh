...
if [ ! -f ~/project_btas/log.txt ]
then
#Log File not present Create it
touch ~/project_btas/log.txt
fi


#Generating Final Report File :
if [ ! -f $location/report.txt ]
then
touch $location/report.txt
fi
...
echo "CDR Analysis in File : $myFile " >> ~/project_btas/log.txt

mylist=`grep -n -c $pno  first.csv`
mylines=`grep -n $pno first.csv`


#Log File writing

echo "Total Number of Lines Phone found : $mylist" >> ~/project_btas/log.txt
echo "Lines Containing the phone number :\n $mylines" >> ~/project_btas/log.txt

echo "Call Details -------------------------------------------------------------------------------" >> ~/project_btas/log.txt
...
then
#pno not present
echo "Number of occurence of $pno  in  $file : 0 \t\t\t  NOT FOUND" >> $location/report.txt
echo " $file Analysis Closed $pno not found " >> ~/project_btas/log.txt
...
echo "File $myFile Analysis : ---------------------------------------------------------------------------------------------------" >> $location/report.txt
cnt=0
...
echo "Call End Time Noted Format YYYYMMDDHHMMSSMMM   \t\t : $wordEg " >> ~/project_btas/log.txt

if [ $wordEg -le $end -a $wordEg -ge $start ]
then
echo "Yes This call ongoing  Call End Time : $wordEg" >> ~/project_btas/log.txt
echo "Call ONGOING : \n Call End Time in format YYYYMMDDHHMMSSMMM : $wordEg" >> $location/report.txt
#Extra Information Can be Added
else
echo "No This Call not ongoing : $wordEg" >> ~project_btas/log.txt
fi
fi
;;
esac
